const App = {
  ...
  async init() {
    ...
    this.bindEvents();
    (_a = document.getElementById("form-login")) == null ? void 0 : _a.addEventListener("submit", (e) => this.handleLogin(e));
    (_b = document.getElementById("btn-logout")) == null ? void 0 : _b.addEventListener("click", (e) => this.handleLogout(e));
    (_c = document.getElementById("btn-toggle-password")) == null ? void 0 : _c.addEventListener("click", () => {
    ...
    if (this.isUserLoggedIn()) {
      ...
      this.showToast("Sess\xE3o Ativa", `Bem-vindo de volta, ${nomeAdmin}!`, "success");
    } else {
  ...
  async handleLogin(e) {
    ...
    if (!user || !pass) {
      this.showToast("Campos Vazios", "Por favor, digite o usu\xE1rio e a senha.", "warning");
      return;
    }
    const defaultUsername = ((_a = window.ENV) == null ? void 0 : _a.DEFAULT_USERNAME) || "admin";
    const defaultPassword = (_b = window.ENV) == null ? void 0 : _b.DEFAULT_PASSWORD;
    if (user.toLowerCase() === defaultUsername.toLowerCase() && !defaultPassword && !DB.isSupabaseActive()) {
      this.showToast("Erro de Configura\xE7\xE3o", "A senha padr\xE3o n\xE3o foi configurada no Cloudflare Pages. Por favor, adicione VITE_DEFAULT_PASSWORD nas configura\xE7\xF5es.", "danger");
      return;
    }
    try {
      ...
      if (resposta.sucesso) {
        ...
        this.exibirPainelAdmin();
        this.showToast("Login Efetuado", `Ol\xE1, ${resposta.usuario.nome || "Administrador"}! Acesso autorizado.`, "success");
      } else {
        this.showToast("Acesso Negado", "Usu\xE1rio ou senha incorretos. Tente novamente.", "danger");
      }
    } catch (err) {
      console.error("Erro na autentica\xE7\xE3o:", err);
      this.showToast("Erro de Conex\xE3o", "N\xE3o foi poss\xEDvel validar o acesso.", "danger");
    }
  },
  handleLogout(e) {
    e.preventDefault();
  ...
  atualizarDashboard() {
    ...
    const colaresVendidos = vendasMes.filter((v) => v.produto_nome.toLowerCase().includes("colar") || v.produtos && v.produtos.categoria === "Colar").reduce((sum, v) => sum + v.quantidade, 0);
    ...
    this.setDOMText("kpi-colares", `${colaresVendidos} pe\xE7as`);
  ...
  gerarInsightsAutom\u00E1ticos(vendas, faturamentoMaio, lucroMaio, faturamentoAtacadoMaio) {
    ...
    if (insumosCriticos.length > 0) {
      insumosCriticos.forEach((insumo) => {
        ...
        div.innerHTML = `
          ...
          <strong>ALERTA DE RUPTURA:</strong> Estoque cr\xEDtico de <strong>${insumo.nome} (${insumo.especificacao})</strong>. Restam apenas <strong>${insumo.estoque_atual} ${insumo.unidade_medida === "metro" ? "m" : insumo.unidade_medida === "grama" ? "g" : "un"}</strong> (Limite m\xEDnimo: ${insumo.estoque_minimo}).
        `;
    ...
    if (faturamentoAbril > 0) {
      ...
      div.innerHTML = `
        ...
        Suas vendas <strong>${isPos ? "cresceram" : "ca\xEDram"} ${Math.abs(pctFaturamento).toFixed(1)}%</strong> em faturamento este m\xEAs comparado a Abril (De R$ ${faturamentoAbril.toFixed(0)} para R$ ${faturamentoMaio.toFixed(0)}).
      `;
      container.appendChild(div);
    }
    if (lucroAbril > 0) {
      ...
      div.innerHTML = `
        ...
        O lucro l\xEDquido do neg\xF3cio <strong>${isPos ? "subiu" : "caiu"} ${Math.abs(pctLucro).toFixed(1)}%</strong> comparado a Abril. Foco na ${isPos ? "manuten\xE7\xE3o do ritmo" : "redu\xE7\xE3o de custos"}.
      `;
      container.appendChild(div);
    }
    const ranking = {};
    ...
    if (topProduto) {
      ...
      div.innerHTML = `
        ...
        O <strong>${topProduto[0]}</strong> foi o seu produto mais vendido este m\xEAs, com um total de <strong>${topProduto[1]} unidades</strong>.
      `;
      container.appendChild(div);
    }
  ...
  responderPerguntaDeNegocio(perguntaKey) {
    ...
    switch (perguntaKey) {
      case "colar": {
        ...
        if (topColar) {
          resposta = `O colar mais vendido deste m\xEAs foi o <strong>"${topColar[0]}"</strong>, com <strong>${topColar[1]} pe\xE7as</strong> comercializadas.`;
        } else {
          resposta = "Sem registros de colares vendidos neste m\xEAs.";
        }
        break;
      }
      case "lucro_peca": {
        ...
        if (topLucro) {
          resposta = `A pe\xE7a com maior retorno financeiro foi o <strong>"${topLucro[0]}"</strong>, gerando <strong>R$ ${topLucro[1].toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} de lucro l\xEDquido real</strong>.`;
        } else {
          resposta = "Sem dados de lucratividade para o m\xEAs corrente.";
        }
        break;
      }
      case "canal": {
        ...
        if (fatVarejo > fatAtacado) {
          resposta = `Voc\xEA vendeu mais no <strong>Varejo</strong> (R$ ${fatVarejo.toLocaleString("pt-BR", { maximumFractionDigits: 0 })} vs R$ ${fatAtacado.toLocaleString("pt-BR", { maximumFractionDigits: 0 })} do Atacado). O varejo representou <strong>${(fatVarejo / (fatVarejo + fatAtacado) * 100).toFixed(0)}%</strong>.`;
        } else if (fatAtacado > fatVarejo) {
          resposta = `Voc\xEA vendeu mais no <strong>Atacado</strong> (R$ ${fatAtacado.toLocaleString("pt-BR", { maximumFractionDigits: 0 })} vs R$ ${fatVarejo.toLocaleString("pt-BR", { maximumFractionDigits: 0 })} do Varejo). O atacado representou <strong>${(fatAtacado / (fatVarejo + fatAtacado) * 100).toFixed(0)}%</strong>.`;
        } else {
      ...
      case "evolucao_mes": {
        ...
        if (fatAbril > 0) {
          ...
          resposta = "Sem registros do m\xEAs de Abril para fins de compara\xE7\xE3o.";
        }
        break;
      }
      case "categoria": {
        ...
        if (topCat) {
          ...
          resposta = "Sem dados categorizados neste m\xEAs.";
        }
        break;
      }
      case "baixa_margem": {
        ...
        if (baixas.length > 0) {
          ...
          resposta = `H\xE1 <strong>${baixas.length} produto(s)</strong> com margem abaixo do recomendado (<25%): <strong>${nomes}</strong>.`;
        } else {
          resposta = "Parab\xE9ns! <strong>Nenhum produto</strong> cadastrado possui margem de lucro baixa.";
        }
        break;
      }
      case "lucro_liquido": {
        const lucLiq = vendasMaio.reduce((sum, v) => sum + parseFloat(v.lucro), 0);
        resposta = `O lucro l\xEDquido de Maio foi de <strong>R$ ${lucLiq.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong> (j\xE1 descontados os custos de materiais e m\xE3o de obra de cada pe\xE7a).`;
        break;
      }
    }
  ...
  renderizarTabelaProdutos() {
    ...
    filtrados.forEach((p) => {
      ...
      const imgHtml = p.foto ? `<img src="${p.foto}" alt="${p.nome}" class="product-table-avatar" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
           <div class="product-placeholder-avatar" style="display: none;"><i class="fas fa-gem"></i></div>` : `<div class="product-placeholder-avatar" style="display: flex;"><i class="fas fa-gem"></i></div>`;
      const custoTotal = p.custo + p.mao_obra;
      const valorVendaAtacado = p.valor_venda_atacado || 0;
      const lucroAtacado = p.lucro_atacado !== void 0 ? p.lucro_atacado : valorVendaAtacado > 0 ? valorVendaAtacado - custoTotal : 0;
      ...
      tr.innerHTML = `
        ...
        <td style="text-align: center; vertical-align: middle; white-space: nowrap;">
          <button class="btn-action btn-action-edit" data-id="${p.id}" title="Editar Pe\xE7a">
            <i class="fas fa-edit"></i>
          </button>
          <button class="btn-action btn-action-delete" data-id="${p.id}" data-nome="${p.nome}" title="Apagar Pe\xE7a">
  ...
  renderizarTabelaClientes() {
    ...
    filtrados.forEach((c) => {
      ...
      tr.innerHTML = `
        ...
        <td style="font-family: monospace;">${c.telefone || "N\xE3o Informado"}</td>
        <td>
  ...
  bindEvents() {
    ...
    ["btn-abrir-venda-modal", "btn-abrir-venda-modal-sec"].forEach((id) => {
      var _a2;
      (_a2 = document.getElementById(id)) == null ? void 0 : _a2.addEventListener("click", (e) => {
        e.preventDefault();
    ...
    (_a = document.getElementById("kpi-card-faturamento")) == null ? void 0 : _a.addEventListener("click", (e) => {
      e.preventDefault();
    ...
    (_b = document.getElementById("kpi-card-lucro")) == null ? void 0 : _b.addEventListener("click", (e) => {
      e.preventDefault();
    ...
    (_c = document.getElementById("kpi-card-colares")) == null ? void 0 : _c.addEventListener("click", (e) => {
      e.preventDefault();
    ...
    (_d = document.getElementById("kpi-card-ticket")) == null ? void 0 : _d.addEventListener("click", (e) => {
      e.preventDefault();
    ...
    (_e = document.getElementById("kpi-card-varejo")) == null ? void 0 : _e.addEventListener("click", (e) => {
      e.preventDefault();
    ...
    (_f = document.getElementById("kpi-card-atacado")) == null ? void 0 : _f.addEventListener("click", (e) => {
      e.preventDefault();
    ...
    (_g = document.getElementById("kpi-card-produto-top")) == null ? void 0 : _g.addEventListener("click", (e) => {
      e.preventDefault();
    ...
    (_h = document.getElementById("kpi-card-produto-lucro")) == null ? void 0 : _h.addEventListener("click", (e) => {
      e.preventDefault();
    ...
    (_i = document.getElementById("kpi-card-categoria-top")) == null ? void 0 : _i.addEventListener("click", (e) => {
      e.preventDefault();
    ...
    (_j = document.getElementById("btn-abrir-categorias-modal")) == null ? void 0 : _j.addEventListener("click", () => {
    ...
    (_k = document.getElementById("form-nova-categoria")) == null ? void 0 : _k.addEventListener("submit", (e) => this.handleSubmitNovaCategoria(e));
    ...
    if (inputCliente) {
      inputCliente.addEventListener("input", () => {
        ...
        if (clienteEncontrado && clienteEncontrado.tipo_cliente) {
          const hiddenTipo2 = document.getElementById("venda-tipo-cliente");
          if (hiddenTipo2 && hiddenTipo2.value !== clienteEncontrado.tipo_cliente) {
            hiddenTipo2.value = clienteEncontrado.tipo_cliente;
            hiddenTipo2.dispatchEvent(new Event("change"));
            const segmentedButtons2 = document.querySelectorAll(".segmented-control .segmented-btn");
            segmentedButtons2.forEach((b) => {
    ...
    (_l = document.getElementById("form-produto")) == null ? void 0 : _l.addEventListener("submit", (e) => this.handleSubmitProduto(e));
    (_m = document.getElementById("form-produto")) == null ? void 0 : _m.addEventListener("reset", () => {
      ...
      if (elSubmit) elSubmit.textContent = "Cadastrar Pe\xE7a";
    ...
    (_n = document.getElementById("form-venda")) == null ? void 0 : _n.addEventListener("submit", (e) => this.handleSubmitVenda(e));
    (_o = document.getElementById("form-cliente")) == null ? void 0 : _o.addEventListener("submit", (e) => this.handleSubmitCliente(e));
    (_p = document.getElementById("form-supabase-config")) == null ? void 0 : _p.addEventListener("submit", (e) => this.handleSubmitSupabase(e));
    (_q = document.getElementById("btn-desconectar-supa")) == null ? void 0 : _q.addEventListener("click", (e) => this.handleDesconectarSupabase(e));
    (_r = document.querySelector(".btn-cancel-delete")) == null ? void 0 : _r.addEventListener("click", (e) => {
      var _a2;
      e.preventDefault();
      (_a2 = document.getElementById("modal-confirmacao-exclusao")) == null ? void 0 : _a2.classList.remove("active");
    });
    (_s = document.querySelector(".btn-confirm-delete")) == null ? void 0 : _s.addEventListener("click", async (e) => {
      var _a2;
      const id = e.currentTarget.getAttribute("data-id");
      if (id) {
        try {
          ...
          this.renderizarTabelaProdutos();
          (_a2 = document.getElementById("modal-confirmacao-exclusao")) == null ? void 0 : _a2.classList.remove("active");
          this.showToast("Pe\xE7a Apagada", "O produto foi exclu\xEDdo com sucesso.", "success");
          this.atualizarDashboard();
        } catch (err) {
          console.error("Erro ao apagar produto:", err);
          this.showToast("Erro ao Excluir", "N\xE3o foi poss\xEDvel remover o produto.", "danger");
        }
      }
    });
    ...
    (_t = document.getElementById("btn-exportar-csv-vendas")) == null ? void 0 : _t.addEventListener("click", () => this.exportarVendasParaCSV());
    (_u = document.getElementById("btn-imprimir-pdf-vendas")) == null ? void 0 : _u.addEventListener("click", () => window.print());
    (_v = document.getElementById("btn-limpar-vendas")) == null ? void 0 : _v.addEventListener("click", () => {
      var _a2;
      (_a2 = document.getElementById("modal-confirmacao-limpar-vendas")) == null ? void 0 : _a2.classList.add("active");
    });
    const modalLimpar = document.getElementById("modal-confirmacao-limpar-vendas");
    if (modalLimpar) {
      (_w = modalLimpar.querySelector(".btn-close-modal")) == null ? void 0 : _w.addEventListener("click", () => {
        modalLimpar.classList.remove("active");
      });
      (_x = modalLimpar.querySelector(".btn-cancelar-limpar")) == null ? void 0 : _x.addEventListener("click", () => {
        modalLimpar.classList.remove("active");
      });
      (_y = modalLimpar.querySelector(".btn-confirmar-limpar")) == null ? void 0 : _y.addEventListener("click", async () => {
        try {
          await DB.limparTodasVendas();
          this.state.vendas = [];
          this.showToast("Vendas Limpas", "Todas as vendas foram exclu\xEDdas com sucesso!", "success");
          this.atualizarDashboard();
          this.renderizarTabelaVendas();
          modalLimpar.classList.remove("active");
        } catch (err) {
          console.error(err);
          this.showToast("Erro ao Limpar", "N\xE3o foi poss\xEDvel limpar o hist\xF3rico de vendas.", "danger");
        }
      });
    }
    ...
    (_z = document.getElementById("btn-abrir-insumo-modal")) == null ? void 0 : _z.addEventListener("click", () => {
      this.abrirModalInsumo();
    });
    (_A = document.getElementById("form-insumo")) == null ? void 0 : _A.addEventListener("submit", (e) => this.handleSubmitInsumo(e));
    (_B = document.getElementById("form-insumo")) == null ? void 0 : _B.addEventListener("reset", () => {
    ...
    (_C = document.getElementById("btn-add-ficha-material")) == null ? void 0 : _C.addEventListener("click", () => {
    ...
    (_D = document.getElementById("btn-close-modal-selecionar-insumo")) == null ? void 0 : _D.addEventListener("click", () => {
  ...
  addModalTrigger(btnId, modalId, isVenda) {
    var _a;
    (_a = document.getElementById(btnId)) == null ? void 0 : _a.addEventListener("click", () => {
  ...
  bindFiltroInput(id, modulo, chave, callback) {
    var _a;
    (_a = document.getElementById(id)) == null ? void 0 : _a.addEventListener("input", (e) => {
  ...
  bindFiltroSelect(id, modulo, chave, callback) {
    var _a;
    (_a = document.getElementById(id)) == null ? void 0 : _a.addEventListener("change", (e) => {
      this.state.filtros[modulo][chave] = e.target.value;
      callback();
    });
  },
  ...
  abrirModalEdicao(prodId) {
    ...
    const elTitulo = document.getElementById("modal-produto-titulo");
    const elSubmit = document.getElementById("btn-submit-produto");
    if (elTitulo) elTitulo.textContent = "Editar Pe\xE7a";
    if (elSubmit) elSubmit.textContent = "Salvar Altera\xE7\xF5es";
  ...
  sincronizarProdutoSelecionadoFormVenda() {
    ...
    if (prod) {
      ...
      if (stockWarn) {
        ...
        if (prod.estoque < qtd) {
          stockWarn.innerHTML = `<i class="fas fa-triangle-exclamation" style="color: var(--danger)"></i> Saldo em estoque insuficiente! Dispon\xEDvel: ${prod.estoque} un.`;
        } else {
          stockWarn.innerHTML = `<i class="fas fa-check" style="color: var(--accent)"></i> Pe\xE7a em estoque (Dispon\xEDvel: ${prod.estoque} un).`;
        }
      }
  ...
  popularSelectProdutosFormVenda() {
    const select = document.getElementById("venda-produto-id");
    if (!select) return;
    select.innerHTML = '<option value="" disabled selected>Selecione uma pe\xE7a do cat\xE1logo...</option>';
    this.state.produtos.forEach((p) => {
      const opt = document.createElement("option");
      opt.value = p.id;
      opt.textContent = `${p.nome} (Dispon\xEDvel: ${p.estoque} un | R$ ${p.valor_venda.toFixed(2)})`;
  ...
  renderizarListaCategorias() {
    ...
    this.state.categorias.forEach((cat) => {
      ...
      item.innerHTML = `
        ...
        ${isOutros ? `<span style="font-size: 11px; color: var(--text-muted); font-style: italic; padding: 4px 8px;">Sistema</span>` : `<button type="button" class="btn-delete-cat" title="Excluir Categoria" data-nome="${cat}">
            <i class="fas fa-trash"></i>
          </button>`}
      `;
      if (!isOutros) {
  ...
  async handleSubmitNovaCategoria(e) {
    ...
    if (this.state.categorias.map((c) => c.toLowerCase()).includes(nome.toLowerCase())) {
      this.showToast("Categoria j\xE1 existe", `A categoria "${nome}" j\xE1 est\xE1 cadastrada.`, "warning");
      return;
    }
  ...
  async handleDeletarCategoria(nome) {
    ...
    if (produtosNaCategoria.length > 0) {
      mensagemConfirmacao = `Aten\xE7\xE3o: Existem ${produtosNaCategoria.length} produto(s) nesta categoria. Se voc\xEA apag\xE1-la, a categoria deles ser\xE1 alterada para "Outros". Deseja prosseguir?`;
    }
    if (confirm(mensagemConfirmacao)) {
      try {
        ...
        this.showToast("Erro ao remover", "N\xE3o foi poss\xEDvel apagar a categoria.", "danger");
      }
    }
  },
  ...
  async handleSubmitProduto(e) {
    ...
    if (maoObraRaw === "" || isNaN(maoObra)) {
      this.showToast("M\xE3o de Obra Requerida", 'Por favor, defina um valor para o campo "Valor M\xE3o de Obra (R$)".', "warning");
      if (maoObraInput) {
        maoObraInput.focus();
    ...
    if (!nome || !categoria || isNaN(custo) || isNaN(maoObra) || isNaN(valorVenda) || isNaN(valorVendaAtacado)) {
      this.showToast("Formul\xE1rio Inv\xE1lido", "Por favor, preencha todos os dados.", "danger");
      return;
    }
    ...
    try {
      ...
      const msgTitulo = isEdicao ? "Pe\xE7a Atualizada" : "Pe\xE7a Cadastrada";
      ...
      this.showToast("Erro ao Salvar", "N\xE3o foi poss\xEDvel registrar o produto.", "danger");
    }
  },
  async handleSubmitVenda(e) {
    ...
    if (!produtoId || !clienteNome || !dataVenda || isNaN(quantidade) || !pagamento || isNaN(valorVenda)) {
      this.showToast("Campos Inv\xE1lidos", "Por favor, confira os dados da transa\xE7\xE3o.", "danger");
      return;
    }
    const prod = this.state.produtos.find((p) => p.id === produtoId);
    if (prod && prod.estoque < quantidade) {
      if (!confirm(`O estoque atual deste produto \xE9 de ${prod.estoque} pe\xE7as. Deseja continuar e registrar saldo negativo?`)) {
        return;
      }
    }
    const venda = {
    ...
    try {
      ...
      if (prod && prod.composicao && prod.composicao.length > 0) {
        const corCordaIdSelected = (_a = document.getElementById("venda-cor-corda")) == null ? void 0 : _a.value;
        for (const item of prod.composicao) {
          ...
          if (insumo) {
            ...
            if (insumo.estoque_atual <= insumo.estoque_minimo) {
              this.showToast(
                "Alerta de Estoque",
                `A corda "${insumo.nome.replace("Corda ", "")}" atingiu o limite cr\xEDtico (Restam apenas ${insumo.estoque_atual}m)!`,
                "danger"
              );
            }
          }
      ...
      (_b = document.getElementById("modal-venda")) == null ? void 0 : _b.classList.remove("active");
      this.resetarFormVenda();
      this.showToast("Venda Registrada!", `Lan\xE7amento efetuado para ${clienteNome}.`, "success");
      this.atualizarDashboard();
    } catch (err) {
  ...
  async handleSubmitCliente(e) {
    ...
    if (!nome) {
      this.showToast("Preenchimento Inv\xE1lido", "O nome do cliente \xE9 obrigat\xF3rio.", "danger");
      return;
    }
  ...
  async handleSubmitSupabase(e) {
    ...
    if (!url || !key) {
      this.showToast("Credenciais Inv\xE1lidas", "Preencha a URL e a Anon Key.", "danger");
      return;
    }
    ...
    if (sucesso) {
      try {
        ...
        this.atualizarDashboard();
        this.atualizarStatusSupabaseUI();
        this.showToast("Conectado \xE0 Nuvem", "Seu banco de dados agora est\xE1 ativado no Supabase!", "success");
      } catch (err) {
        console.error("Erro ao conectar Supabase:", err);
        this.showToast("Falha na Conex\xE3o", "N\xE3o foi poss\xEDvel sincronizar as tabelas do Supabase.", "danger");
      }
    } else {
      this.showToast("Erro na Conex\xE3o", "N\xE3o foi poss\xEDvel configurar a biblioteca do Supabase.", "danger");
    }
  },
  ...
  atualizarStatusSupabaseUI() {
    ...
    if (dot && text) {
      ...
      if (active) {
        ...
        text.innerHTML = "Conex\xE3o Estabelecida! Operando em <strong>Nuvem (Supabase)</strong>.";
      } else {
  ...
  exportarVendasParaCSV() {
    const vendas = this.state.vendas;
    if (vendas.length === 0) {
      this.showToast("Exporta\xE7\xE3o Inv\xE1lida", "N\xE3o h\xE1 registros de vendas.", "danger");
      return;
    }
    let csvContent = "\uFEFF";
    ...
    const dataHoraStr = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
    link.setAttribute("download", `relatorio_vendas_deuno_${dataHoraStr}.csv`);
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    this.showToast("Download Conclu\xEDdo", "Relat\xF3rio CSV exportado com sucesso!", "success");
  },
  ...
  renderizarTabelaInsumos() {
    ...
    if (!tbody) return;
    tbody.innerHTML = "";
    const busca = (((_a = this.state.filtros.insumos) == null ? void 0 : _a.busca) || "").toLowerCase();
    const tipo = ((_b = this.state.filtros.insumos) == null ? void 0 : _b.tipo) || "todos";
    const filtrados = this.state.insumos.filter((insumo) => {
      const matchBusca = insumo.nome.toLowerCase().includes(busca) || insumo.especificacao.toLowerCase().includes(busca);
    ...
    filtrados.forEach((insumo) => {
      ...
      let statusTexto = "Saud\xE1vel";
      if (estoqueAtual <= estoqueMinimo) {
        badgeClass = "badge-danger";
        statusTexto = "Cr\xEDtico";
  ...
  async handleSubmitInsumo(e) {
    ...
    if (!nome || !tipo || !especificacao || isNaN(precoCusto)) {
      this.showToast("Campos Inv\xE1lidos", "Por favor, confira os dados do material.", "danger");
      return;
    }
    const insumo = {
      nome,
    ...
    try {
      ...
      this.showToast("Erro ao Salvar", "N\xE3o foi poss\xEDvel registrar o material.", "danger");
    }
  },
  async deletarInsumo(id) {
    ...
    if (confirm(`Deseja realmente excluir o material "${insumo.nome} (${insumo.especificacao})"?`)) {
      try {
        ...
        await this.carregarDados();
        this.renderizarTabelaInsumos();
        this.showToast("Material Removido", "O material foi exclu\xEDdo com sucesso.", "success");
        this.atualizarDashboard();
      } catch (err) {
        console.error("Erro ao deletar insumo:", err);
        this.showToast("Erro ao Excluir", "N\xE3o foi poss\xEDvel remover o material.", "danger");
      }
    }
  },
  adicionarLinhaFichaTecnica(insumoId = "", quantidade = 1) {
    ...
    if (this.state.insumos.length === 0) {
      this.showToast("Nenhum Material", "Cadastre materiais na aba de Materiais antes de compor a pe\xE7a!", "warning");
      return;
    }
    ...
    row.innerHTML = `
      <!-- Select real escondido para manter compatibilidade com a l\xF3gica existente -->
    ...
    const atualizarCustoLinha = () => {
      ...
      const preco = parseFloat(selectedOption == null ? void 0 : selectedOption.getAttribute("data-preco")) || 0;
      const labelText = selectedOption ? selectedOption.text : "-- Escolha o Material --";
      const labelEl = row.querySelector(".ficha-insumo-label");
      if (labelEl) {
        labelEl.textContent = labelText.split(" - R$")[0];
      }
      const qtd = parseFloat(qtdEl.value) || 0;
  ...
  renderizarListaBuscaInsumos() {
    ...
    if (!listaEl) return;
    const termo = (((_a = document.getElementById("busca-insumo-termo")) == null ? void 0 : _a.value) || "").toLowerCase().trim();
    ...
    const insumosFiltrados = this.state.insumos.filter((ins) => {
      ...
      const bateTermo = !termo || ins.nome.toLowerCase().includes(termo) || ins.especificacao && ins.especificacao.toLowerCase().includes(termo) || ins.tipo && ins.tipo.toLowerCase().includes(termo);
      ...
      const emptyState = `
        <tr>
          <td colspan="7" class="empty-state" style="text-align: center; padding: 32px; color: var(--text-muted);">
            ...
            Nenhuma transa\xE7\xE3o registrada neste m\xEAs.
          </td>
        </tr>
      `;
      ...
      tr.innerHTML = `
        ...
        <td>
          <span style="font-size: 11px; font-weight: 500; color: var(--text-secondary); display: inline-flex; align-items: center; gap: 4px;">
            <i class="${v.pagamento === "PIX" ? "fab fa-pix" : v.pagamento === "Cart\xE3o" ? "fas fa-credit-card" : "fas fa-money-bill-wave"}" style="font-size: 10px; color: var(--text-muted);"></i>
            ${v.pagamento}
          </span>
        </td>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 4px;">
          <div style="display: flex; gap: 6px;">
            ...
            <span style="font-size: 10px; background: rgba(0,0,0,0.03); padding: 3px 6px; border-radius: 4px; color: var(--text-secondary); display: inline-flex; align-items: center; gap: 3px; border: 1px solid var(--border-dim);">
              <i class="${v.pagamento === "PIX" ? "fab fa-pix" : v.pagamento === "Cart\xE3o" ? "fas fa-credit-card" : "fas fa-money-bill-wave"}" style="font-size: 8px;"></i>
              ${v.pagamento}
            </span>
          </div>
    ...
    vendasOrdenadas.forEach((v) => {
      ...
      card.innerHTML = `
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 4px; border-top: 1px solid rgba(0,0,0,0.03); padding-top: 6px;">
          <span style="font-size: 11px; color: var(--text-secondary); font-weight: 500;">Lucro L\xEDquido Real:</span>
  ...
  renderizarColaresDetalhado() {
    ...
    this.setDOMText("det-col-pecas", `${totalPecas} pe\xE7as`);
      ...
      const emptyState = `
        <tr>
          <td colspan="8" class="empty-state" style="text-align: center; padding: 32px; color: var(--text-muted);">
            ...
            Nenhum colar vendido neste m\xEAs.
          </td>
        </tr>
      `;
      tbody.innerHTML = emptyState;
    ...
    ordenadas.forEach((v) => {
      ...
      card.innerHTML = `
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid rgba(0,0,0,0.03); padding-top: 6px;">
          <span style="font-size: 11px; color: var(--text-secondary); font-weight: 500;">Lucro L\xEDquido:</span>
          <span style="font-weight: 700; color: var(--success); font-size: 14px;">R$ ${lucroReal.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
        </div>
      `;
      mobileContainer.appendChild(card);
    ...
    ranking.forEach((p, idx) => {
      ...
      card.innerHTML = `
        ...
        <div style="display: flex; gap: 10px; align-items: center;">
          ...
          <div style="flex: 1; min-width: 0;">
            <div style="font-weight: 600; font-size: 13px; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${p.nome}</div>
            <div style="font-size: 11px; color: var(--text-secondary); margin-top: 2px;">Pe\xE7as compradas &bull; ${p.quantidade} un</div>
          </div>
        </div>
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 4px; font-size: 11px;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid rgba(0,0,0,0.03); padding-top: 6px; font-size: 11px;">
          <span style="color: var(--text-secondary); font-weight: 500;">Lucro L\xEDquido Acumulado:</span>
  ...
  renderizarCategoriaTopDetalhado() {
    ...
    ranking.forEach((c, idx) => {
      ...
      tr.innerHTML = `
        ...
        <td style="text-align: right; font-weight: 500;">${c.quantidade} pe\xE7as</td>
      ...
      card.innerHTML = `
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; font-size: 11px;">
          ...
          <span style="font-weight: 600; color: var(--text-primary);">${c.quantidade} pe\xE7as</span>
        </div>
        <div style="display: flex; justify-content: space-between; align-items: center; font-size: 11px;">
          <span style="color: var(--text-secondary);">Faturamento Total:</span>
